...
public class Font {
   ...
   public void render(CharSequence text, float x, float y, Vec4 color) {
       ...
       for (int i = 0; i < text.length(); i++) {
           ...
           ResourceManager.getSelf().getTextureRenderer().render(texture.getId(), new Vec2(drawX,drawY), new Vec2(g.width,g.height), 0, color, frame, new Vec2(0,1), new Vec2(0,0));
           drawX += g.width;
       }
      // renderer.end();
   }
